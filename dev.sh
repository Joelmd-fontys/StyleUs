...
start_worker_service() {
  log "Starting worker service (FastAPI @ http://localhost:8001)..."
  (
    cd "$API_DIR"
    exec "$VENV_DIR/bin/uvicorn" app.worker_service:app --host 0.0.0.0 --port 8001
  ) &
  WORKER_PID=$!
}
...
main() {
  ...
  log "All set. API: http://localhost:8000 | Worker: http://localhost:8001/health | Web: http://localhost:5173"
