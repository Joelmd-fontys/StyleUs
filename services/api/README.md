...
- `make typecheck`
- `make test`
- `make benchmark-category` - run the upload-style offline category benchmark against `tests/fixtures/upload_category_cases.yaml`
- `make review-feedback-report` - replay the review-feedback correction slice and print confusion plus threshold guidance
- `make review-feedback-export` - export the latest upload-review corrections from the database into `media/review_feedback_eval/`
...
- `AI_MODEL_NAME`
- `AI_MODEL_PRETRAINED`
- `AI_CLASSIFICATION_INPUT`
- `AI_MODEL_CACHE_DIR`
- `AI_CONFIDENCE_THRESHOLD`
- `AI_ACCESSORY_CONFIDENCE_THRESHOLD`
...
- Runtime: `Docker`
- Plan: `starter` for the API and `standard` for the AI worker in the default Render blueprint
...
- The base runtime dependencies now include the lightweight heuristic AI stack (`numpy` and `scikit-learn`) because the API still keeps a local fallback path.
...
- The Render blueprint keeps `AI_ENABLE_CLASSIFIER=true` in production and pins `AI_CLASSIFICATION_INPUT=full` so hosted inference matches the benchmarked category path.
- The worker and API share `AI_ACCESSORY_CONFIDENCE_THRESHOLD=0.62` so mid-confidence accessory predictions stay review-recommended instead of looking final.
...
- In production, upload completion should queue classifier work instead of relying on heuristic-only enrichment.
- The worker stores item embeddings and confidence-aware preview payloads for later similarity search and manual review.
- Measured locally, that heuristic path uses about `288 MB` RSS and finishes in about `1.4s` on a sample image.
- Measured locally, the current `ViT-B-32` worker warmup reaches about `1600 MB` RSS, so production should keep the worker on a higher-memory plan.
